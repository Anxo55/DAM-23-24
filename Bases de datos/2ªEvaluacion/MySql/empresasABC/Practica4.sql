...
SELECT AVG(sueldo) SueldoMedio, SUM(sueldo) SumaTotal
FROM Empleados;

#2) Hallar el importe medio de pedidos, el importe total de pedidos y el precio medio de venta.
SELECT AVG(cantidad*PrecioVenta), SUM(cantidad*PrecioVenta)
FROM LineasPedido;

#3) Hallar el precio medio de los productos del fabricante ACI.
SELECT AVG(PrecioCompra) PrecioMedio
FROM Productos
WHERE IdFabricante LIKE "ACI%";
